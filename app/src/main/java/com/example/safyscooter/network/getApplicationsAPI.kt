                            ...
                            for (j in 0 until verdictsArray.length()) {
                                ...
                                verdictsList.add(
                                    Verdict(
                                        ...
                                        scooterType = if (verdictJson.has("scooter_type") && !verdictJson.isNull(
                                                "scooter_type"
                                            )
                                        )
                                            verdictJson.getString("scooter_type") else null,
                                        objectId = if (verdictJson.has("object_id") && !verdictJson.isNull(
                                                "object_id"
                                            )
                                        )
                                            verdictJson.getString("object_id") else null,
                                        timestamp = if (verdictJson.has("timestamp") && !verdictJson.isNull(
                                                "timestamp"
                                            )
                                        )
                                            verdictJson.getDouble("timestamp") else 0.0,
                                        coordinates = coordinatesList,
                                        createdAt = if (verdictJson.has("created_at") && !verdictJson.isNull(
                                                "created_at"
                                            )
                                        )
